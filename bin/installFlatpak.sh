...
if [ -e "$HOME/.fedora/flatpak-remove.conf" ]
then
	cat "$HOME/.fedora/flatpak-remove.conf" | grep -v '^#' | grep -v '^$' | \
	while read LINE
	do
...
if [ -e "$HOME/.fedora/flatpak-install.conf" ]
then
	cat "$HOME/.fedora/flatpak-install.conf" | grep -v '^#' | grep -v '^$' | \
	while read LINE
	do
